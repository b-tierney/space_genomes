...
cat genomes/*_reformatted  > all_contigs.fna

anvi-gen-contigs-database -T 8 -f all_contigs.fna -o contigs.db -n 'PDS08 bifido isolate genomes'
...
anvi-import-collection contigs.txt \
                       -c contigs.db \
                       -p pds08-bifido-MERGED/PROFILE.db \
                       -C Genomes
...
anvi-summarize -c contigs.db \
               ...
               -C Genomes \
               --init-gene-coverages \
               -o pds08-bifido-SUMMARY

./get_internal_genome_files.sh

anvi-gen-genomes-storage -i internal_genomes.txt \
                         -o pds08-bifido-PAN-GENOMES.db

anvi-pan-genome -g pds08-bifido-PAN-GENOMES.db \
                --use-ncbi-blast \
                --minbit 0.5 \
                --mcl-inflation 10 \
                --project-name pds08-bifido-PAN \
                --num-threads 20

anvi-meta-pan-genome -i internal_genomes.txt \
                                           -p pds08-bifido-PAN/pds08-bifido-PAN-PAN.db \
                                           -g pds08-bifido-PAN-GENOMES.db \
...
anvi-script-gen-distribution-of-genes-in-a-bin -p pds08-bifido-MERGED/PROFILE.db \
                                               -c contigs.db \
                                               -C Genomes \
                                               --fraction-of-median-coverage 0.25 \
                                               -b "$p" 
