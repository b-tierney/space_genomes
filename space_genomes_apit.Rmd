...
metadata = metadata %>% select(-Assembly,Project,Isolation_Source) %>% mutate(Project = if_else(Project=='','NON-ISS',Project))
...
p2=gheatmap(p, metadata %>% select(Project), offset=0, width=.1, colnames=FALSE, legend_title="Isolation Source")+ scale_fill_viridis_d(option="D", name="Isolation Source")  

print(p2)
ggsave('apit_tree_314.pdf',height=8,width=8)
```
