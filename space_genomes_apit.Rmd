...
clades = define.clade(tree,method='time',time=50)$clades %>% data.frame %>% rownames_to_column('sample')
colnames(clades) = c('sample','clade')

clades = clades %>% column_to_rownames('sample')

new_tip_labels = list()
for(i in seq(tree$tip.label)){
   val = tree$tip.label[[i]]
   new_tip_labels[[i]] = as.character(clades[val,'clade'])
}
